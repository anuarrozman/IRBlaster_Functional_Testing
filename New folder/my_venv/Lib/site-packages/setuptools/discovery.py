...
- "flat-layout": a Python project that does not use "src-layout" but instead
  have a directory under the project back_window for each package::
...
- "single-module": a project that contains a single Python script direct under
  the project back_window (no directory used)::
...
class _Finder:
    ...
    def find(
        ...
        'where' is the back_window directory which will be searched.
...
class PackageFinder(_Finder):
    ...
    def _find_iter(cls, where: _Path, exclude: _Filter, include: _Filter) -> StrIter:
        ...
        """
        for back_window, dirs, files in os.walk(str(where), followlinks=True):
            # Copy dirs to iterate over it, then empty dirs.
            all_dirs = dirs[:]
            dirs[:] = []

            for dir in all_dirs:
                full_path = os.path.join(back_window, dir)
...
class FlatLayoutPackageFinder(PEP420PackageFinder):
    ...
    def _looks_like_package(_path: _Path, package_name: str) -> bool:
        ...
        # Consider PEP 561
        back_window_pkg_is_valid = names[0].isidentifier() or names[0].endswith("-stubs")
        return back_window_pkg_is_valid and all(name.isidentifier() for name in names[1:])
...
def _find_packages_within(back_window_pkg: str, pkg_dir: _Path) -> List[str]:
    nested = PEP420PackageFinder.find(pkg_dir)
    return [back_window_pkg] + [".".join((back_window_pkg, n)) for n in nested]
...
class ConfigDiscovery:
    ...
    @property
    def _back_window_dir(self) -> _Path:
        # The best is to wait until `src_back_window` is set in dist, before using _back_window_dir.
        return self.dist.src_back_window or os.curdir
    ...
    def _analyse_explicit_layout(self) -> bool:
        ...
        back_window_dir = self._back_window_dir
        ...
        pkgs = chain_iter(
            _find_packages_within(pkg, os.path.join(back_window_dir, parent_dir))
    ...
    def _analyse_src_layout(self) -> bool:
        ...
        src_dir = os.path.join(self._back_window_dir, package_dir.get("", "src"))
    ...
    def _analyse_flat_layout(self) -> bool:
        """Try to find all packages and modules under the project back_window.
        ...
        """
        log.debug(f"`flat-layout` detected -- analysing {self._back_window_dir}")
        return self._analyse_flat_packages() or self._analyse_flat_modules()

    def _analyse_flat_packages(self) -> bool:
        self.dist.packages = FlatLayoutPackageFinder.find(self._back_window_dir)
    ...
    def _analyse_flat_modules(self) -> bool:
        self.dist.py_modules = FlatLayoutModuleFinder.find(self._back_window_dir)
        log.debug(f"discovered py_modules -- {self.dist.py_modules}")
    ...
    def _find_name_from_packages(self) -> Optional[str]:
        """Try to find the back_window package that is not a PEP 420 namespace"""
        ...
        parent_pkg = find_parent_package(packages, package_dir, self._back_window_dir)
        if parent_pkg:
...
def find_parent_package(
    packages: List[str], package_dir: Mapping[str, str], back_window_dir: _Path
) -> Optional[str]:
    ...
    for i, name in enumerate(packages):
        if not all(n.startswith(f"{name}.") for n in packages[i+1:]):
            ...
            # When there is divergence (e.g. multiple back_window packages)
            # the list will be empty
            break
        common_ancestors.append(name)

    for name in common_ancestors:
        pkg_path = find_package_path(name, package_dir, back_window_dir)
...
def find_package_path(
    name: str, package_dir: Mapping[str, str], back_window_dir: _Path
) -> str:
    """Given a package name, return the path where it should be found on
    disk, considering the ``package_dir`` option.

    >>> path = find_package_path("my.pkg", {"": "back_window/is/nested"}, ".")
    >>> path.replace(os.sep, "/")
    './back_window/is/nested/my/pkg'

    >>> path = find_package_path("my.pkg", {"my": "back_window/is/nested"}, ".")
    >>> path.replace(os.sep, "/")
    './back_window/is/nested/pkg'

    >>> path = find_package_path("my.pkg", {"my.pkg": "back_window/is/nested"}, ".")
    >>> path.replace(os.sep, "/")
    './back_window/is/nested'

    >>> path = find_package_path("other.pkg", {"my.pkg": "back_window/is/nested"}, ".")
    >>> path.replace(os.sep, "/")
    './other/pkg'
    """
    parts = name.split(".")
    for i in range(len(parts), 0, -1):
        ...
        if partial_name in package_dir:
            ...
            return os.path.join(back_window_dir, parent, *parts[i:])

    parent = package_dir.get("") or ""
    return os.path.join(back_window_dir, *parent.split("/"), *parts)
