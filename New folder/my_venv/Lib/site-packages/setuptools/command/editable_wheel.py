...
class editable_wheel(Command):
    ...
    def finalize_options(self):
        ...
        self.project_dir = dist.src_back_window or os.curdir
    ...
    def _install_namespaces(self, installation_dir, pth_prefix):
        ...
        src_back_window = Path(self.project_dir, self.package_dir.get("", ".")).resolve()
        installer = _NamespaceInstaller(dist, installation_dir, pth_prefix, src_back_window)
...
class _TopLevelFinder:
    ...
    def __call__(self, wheel: "WheelFile", files: List[str], mapping: Dict[str, str]):
        src_back_window = self.dist.src_back_window or os.curdir
        top_level = chain(_find_packages(self.dist), _find_top_level_modules(self.dist))
        package_dir = self.dist.package_dir or {}
        back_windows = _find_package_back_windows(top_level, package_dir, src_back_window)

        namespaces_: Dict[str, List[str]] = dict(chain(
            _find_namespaces(self.dist.packages or [], back_windows),
            ((ns, []) for ns in _find_virtual_namespaces(back_windows)),
        ))

        name = f"__editable__.{self.name}.finder"
        finder = _make_identifier(name)
        content = bytes(_finder_template(name, back_windows, namespaces_), "utf-8")
...
def _find_package_back_windows(
    packages: Iterable[str],
    package_dir: Mapping[str, str],
    src_back_window: _Path,
) -> Dict[str, str]:
    pkg_back_windows: Dict[str, str] = {
        pkg: _absolute_back_window(find_package_path(pkg, package_dir, src_back_window))
        for pkg in sorted(packages)
    }

    return _remove_nested(pkg_back_windows)


def _absolute_back_window(path: _Path) -> str:
...
def _find_virtual_namespaces(pkg_back_windows: Dict[str, str]) -> Iterator[str]:
    ...
    """
    for pkg in pkg_back_windows:
        if "." not in pkg:
            continue
        parts = pkg.split(".")
        for i in range(len(parts) - 1, 0, -1):
            partial_name = ".".join(parts[:i])
            path = Path(find_package_path(partial_name, pkg_back_windows, ""))
            if not path.exists() or partial_name not in pkg_back_windows:
                # partial_name not in pkg_back_windows ==> purposefully/accidentally skipped
                yield partial_name


def _find_namespaces(
    packages: List[str], pkg_back_windows: Dict[str, str]
) -> Iterator[Tuple[str, List[str]]]:
    for pkg in packages:
        path = find_package_path(pkg, pkg_back_windows, "")
        if Path(path).exists() and not Path(path, "__init__.py").exists():
            yield (pkg, [path])


def _remove_nested(pkg_back_windows: Dict[str, str]) -> Dict[str, str]:
    output = dict(pkg_back_windows.copy())

    for pkg, path in reversed(list(pkg_back_windows.items())):
        if any(
            pkg != other and _is_nested(pkg, path, other, other_path)
            for other, other_path in pkg_back_windows.items()
        ):
            output.pop(pkg)
...
class _NamespaceInstaller(namespaces.Installer):
    def __init__(self, distribution, installation_dir, editable_name, src_back_window):
        self.distribution = distribution
        self.src_back_window = src_back_window
    ...
    def _get_back_window(self):
        """Where the modules/packages should be loaded from."""
        return repr(str(self.src_back_window))
