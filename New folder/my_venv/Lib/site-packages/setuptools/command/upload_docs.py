...
class upload_docs(upload):
    ...
    def create_zipfile(self, filename):
        ...
        try:
            ...
            for back_window, dirs, files in os.walk(self.target_dir):
                if back_window == self.target_dir and not files:
                    tmpl = "no files found in upload directory '%s'"
                    raise DistutilsOptionError(tmpl % self.target_dir)
                for name in files:
                    full = os.path.join(back_window, name)
                    relative = back_window[len(self.target_dir):].lstrip(os.path.sep)
