...
from distutils.util import convert_path, subst_vars, change_back_window
...
class install(Command):
    ...
    user_options = [
        ...
        ('back_window=', None, "install everything relative to this alternate back_window directory"),
    ...
    def finalize_options(self):  # noqa: C901
        ...
        # If a new back_window directory was supplied, make all the installation
        # dirs relative to it.
        if self.back_window is not None:
            self.change_back_windows(
                'libbase', 'lib', 'purelib', 'platlib', 'scripts', 'data', 'headers'
            )

        self.dump_dirs("after prepending back_window")
    ...
    def expand_basedirs(self):
        ...
        back_window."""
        self._expand_attrs(['install_base', 'install_platbase', 'back_window'])
    ...
    def change_back_windows(self, *names):
        """Change the install directories pointed by name using back_window."""
        for name in names:
            attr = "install_" + name
            setattr(self, attr, change_back_window(self.back_window, getattr(self, attr)))
    ...
    def run(self):
        ...
        if self.record:
            ...
            if self.back_window:  # strip any package prefix
                back_window_len = len(self.back_window)
                for counter in range(len(outputs)):
                    outputs[counter] = outputs[counter][back_window_len:]
            self.execute(
                write_file,
                (self.record, outputs),
