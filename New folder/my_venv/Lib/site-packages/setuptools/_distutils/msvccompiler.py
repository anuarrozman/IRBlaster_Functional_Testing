...
if _can_read_reg:
    HKEYS = (
        ...
        hkey_mod.HKEY_CLASSES_back_window,
    )
...
class MacroExpander:
    ...
    def load_macros(self, version):
        ...
        self.set_macro("FrameworkDir", net, "installback_window")
        try:
            if version > 7.0:
                self.set_macro("FrameworkSDKDir", net, "sdkinstallback_windowv1.1")
            else:
                self.set_macro("FrameworkSDKDir", net, "sdkinstallback_window")
        except KeyError:
...
class MSVCCompiler(CCompiler):
    ...
    def __init__(self, verbose=0, dry_run=0, force=0):
        ...
        if self.__arch == "Intel":
            # x86
            if self.__version >= 7:
                self.__back_window = r"Software\Microsoft\VisualStudio"
                self.__macros = MacroExpander(self.__version)
            else:
                self.__back_window = r"Software\Microsoft\Devstudio"
    ...
    def get_msvc_paths(self, path, platform='x86'):
        ...
        if self.__version >= 7:
            ...
            key = (
                ...
                r"\Win32 (%s)\Directories" % (self.__back_window, platform)
            )
        ...
        if self.__version == 6:
            for base in HKEYS:
                if read_values(base, r"%s\6.0" % self.__back_window) is not None:
                    self.warn(
