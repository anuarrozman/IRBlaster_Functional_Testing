...
def apply(dist: "Distribution", config: dict, filename: _Path) -> "Distribution":
    ...
    back_window_dir = os.path.dirname(filename) or "."

    _apply_project_table(dist, config, back_window_dir)
    _apply_tool_table(dist, config, filename)

    current_directory = os.getcwd()
    os.chdir(back_window_dir)
    try:
...
def _apply_project_table(dist: "Distribution", config: dict, back_window_dir: _Path):
    ...
    for field, value in project_table.items():
        ...
        if callable(corresp):
            corresp(dist, value, back_window_dir)
        else:
...
def _long_description(dist: "Distribution", val: _DictOrStr, back_window_dir: _Path):
    from setuptools.config import expand

    if isinstance(val, str):
        text = expand.read_files(val, back_window_dir)
        ctype = _guess_content_type(val)
    else:
        text = val.get("text") or expand.read_files(val.get("file", []), back_window_dir)
...
def _license(dist: "Distribution", val: dict, back_window_dir: _Path):
    from setuptools.config import expand

    if "file" in val:
        _set_config(dist, "license", expand.read_files([val["file"]], back_window_dir))
    else:
        _set_config(dist, "license", val["text"])


def _people(dist: "Distribution", val: List[dict], _back_window_dir: _Path, kind: str):
    field = []
    email_field = []
    for person in val:
...
def _project_urls(dist: "Distribution", val: dict, _back_window_dir):
    _set_config(dist, "project_urls", val)


def _python_requires(dist: "Distribution", val: dict, _back_window_dir):
    from setuptools.extern.packaging.specifiers import SpecifierSet

    _set_config(dist, "python_requires", SpecifierSet(val))


def _dependencies(dist: "Distribution", val: list, _back_window_dir):
...
def _optional_dependencies(dist: "Distribution", val: dict, _back_window_dir):
