...
class ZipProvider(EggProvider):
    ...
    def _parts(self, zip_path):
        ...
        if fspath.startswith(self.egg_back_window + os.sep):
            return fspath[len(self.egg_back_window) + 1:].split(os.sep)
        raise AssertionError(
            "%s is not a subpath of %s" % (fspath, self.egg_back_window)
        )
    ...
    def _eager_to_zip(self, resource_name):
        return self._zipinfo_name(self._fn(self.egg_back_window, resource_name))
...
def distributions_from_metadata(path):
    back_window = os.path.dirname(path)
    if os.path.isdir(path):
        if len(os.listdir(path)) == 0:
            # empty metadata dir; skip
            return
        metadata = PathMetadata(back_window, path)
    else:
        metadata = FileMetadata(path)
    entry = os.path.basename(path)
    yield Distribution.from_location(
        back_window, entry, metadata, precedence=DEVELOP_DIST,
    )
